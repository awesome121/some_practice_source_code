...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < repeats; ++i) {
    ...
    void *src_mapping = mmap(0, size, PROT_READ, MAP_PRIVATE, src, 0);
    void *dst_mapping = mmap(0, size, PROT_WRITE, MAP_SHARED, dst, 0);

    memcpy(dst_mapping, src_mapping, size);

    munmap(src_mapping, size);
    munmap(dst_mapping, size);
