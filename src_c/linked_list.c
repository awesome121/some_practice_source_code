...
struct Link *map_list(struct Link* head, int (*mapping_func)()) {
    struct Link* new_head = malloc(sizeof(struct Link));
    struct Link* current_link = new_head;
    for (struct Link* l = head; l != NULL; l = l->next) {
        current_link->value = mapping_func(l->value);
        if (l->next != NULL) {
            current_link->next = malloc(sizeof(struct Link));
        }
...
void free_list(struct Link *list) {
    ...
    struct Link* tmp;
    for (struct Link* l = list; l != NULL; l = tmp) {
        tmp = l->next;
        free(l);
    }
