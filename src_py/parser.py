...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        while self.current_char_index < len(self.input_string) and (self.input_string[self.current_char_index] == ' ' or self.input_string[self.current_char_index] == '\n' or self.input_string[self.current_char_index] == '\t'):
    ...
    def get_token(self):
        ...
        if len(self.input_string[self.current_char_index:]) > 0 and token is None:
            self.no_token()
        # consume the token by moving the index to the end of the matched part
        self.current_char_index += len(longest)
        return (token, longest)

    def lookahead(self):# called outside
    ...
    def consume(self, *expected_tokens):# called outside
...
def statement():
    ...
    if scanner.lookahead() == Token.IF:
        ...
        return scanner.consume(Token.IF, Token.WHILE, Token.ID, Token.WRITE)
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        els = statements()
        result = If_Else_AST(condition, then, els)
    else:
        result = If_AST(condition, then)
