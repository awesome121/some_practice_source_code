...
class MinHeap(Heap):
    ...
    def pop_min(self):
        ...
        """
        # ---start student section---
        if len(self._items) - 1 == 0:
            return None
        else:
            removed = self._items[1]
    ...
    def _sift_down(self, index):
        ...
        if (index * 2) <= len(self):
            left = 2 * index
            right = left + 1
            if right <= len(self) and self._items[left] >= self._items[right]:
                smallest = right
            else:
                smallest = left
    ...
    def validate(self):
        ...
        # ---start student section---
        parent_index = 1
        left_child_index, right_child_index = parent_index * 2, parent_index * 2 + 1
        while left_child_index <= len(self._items)-1:
            if self._items[parent_index] > self._items[left_child_index]:
                return False
            if right_child_index <= len(self._items)-1:
                if self._items[parent_index] > self._items[right_child_index]:
                    return False
            parent_index += 1
            left_child_index, right_child_index = parent_index * 2, parent_index * 2 + 1
        return True
