...
class NicerUnsortedFreqList(FreqList):
    ...
    def add(self, new_item):
        ...
        elif self.head != None and not found:
            pre_node.next_node = FreqNode(new_item)
        # ===end student section===
    ...
    def add(self, new_item):
        ...
        while current != None and not found:
            if current.item == new_item:
                current.increment()
                found = True
                if previous != None and previous.frequency < current.frequency:
        ...
        if previous == None and not found:
...
def run_a_test(doc, freq_class, n_chars, verbose=True):
    ...
    if verbose:
        print(freq_list)
        print(freq_list.get_item_frequency('ebo'))
...
def run_tests(filename, verbose=True):
    ...
    run_settings.append((NicerUnsortedFreqList, 3, verbose))
...
if __name__ == '__main__':
    ...
    # Idividual tests:
    #doctest.run_docstring_examples(UnsortedFreqList, None)
    #doctest.run_docstring_examples(NicerUnsortedFreqList, None)
    #doctest.run_docstring_examples(SortedFreqList, None)
